...
final static Keyword SHARED_KEY = Keyword.intern(null, "tcshared");
final static public Var AGENT = Var.intern(CLOJURE_NS, Symbol.create("*agent*"), null, false);
final static public Var READEVAL = Var.intern(CLOJURE_NS, Symbol.create("*read-eval*"), T, false);
final static public Var MACRO_META = Var.intern(CLOJURE_NS, Symbol.create("*macro-meta*"), null, false);
final static public Var MATH_CONTEXT = Var.intern(CLOJURE_NS, Symbol.create("*math-context*"), null, false);
static Keyword LINE_KEY = Keyword.intern(null, "line");
static Keyword FILE_KEY = Keyword.intern(null, "file");
final static public Var USE_CONTEXT_CLASSLOADER =
		Var.intern(CLOJURE_NS, Symbol.create("*use-context-classloader*"), null, false);
...
final static Var CMD_LINE_ARGS = Var.intern(CLOJURE_NS, Symbol.create("*command-line-args*"), null, false);
//symbol
final public static Var CURRENT_NS = Var.intern(CLOJURE_NS, Symbol.create("*ns*"),
                                                CLOJURE_NS, false);

final static Var FLUSH_ON_NEWLINE = Var.intern(CLOJURE_NS, Symbol.create("*flush-on-newline*"), T, false);
final static Var PRINT_META = Var.intern(CLOJURE_NS, Symbol.create("*print-meta*"), F, false);
final static Var PRINT_READABLY = Var.intern(CLOJURE_NS, Symbol.create("*print-readably*"), T, false);
final static Var PRINT_DUP = Var.intern(CLOJURE_NS, Symbol.create("*print-dup*"), F, false);
final static Var WARN_ON_REFLECTION = Var.intern(CLOJURE_NS, Symbol.create("*warn-on-reflection*"), F, false);
final static Var ALLOW_UNRESOLVED_VARS = Var.intern(CLOJURE_NS, Symbol.create("*allow-unresolved-vars*"), F, false);

final static Var IN_NS_VAR = Var.intern(CLOJURE_NS, Symbol.create("in-ns"), F, false);
...
static void doInit() throws Exception{
	if (INITIALIZED == null) {
		...
		exclude.add(Symbol.intern("*out*"));
		exclude.add(Symbol.intern("*in*"));
		exclude.add(Symbol.intern("*err*"));
		shareNamespace(CLOJURE_NS, exclude);
		shareNamespace(Namespace.findOrCreate(Symbol.create("clojure.main")));

		INITIALIZED = Boolean.TRUE;
	}
}

static void shareNamespace(Namespace ns) throws Exception {
	shareNamespace(ns, new ArrayList());
}

static void shareNamespace(Namespace ns, List exclude) throws Exception {
	Iterator itr = ns.getMappings().iterator();
	...
	while(itr.hasNext()) {
		...
		if (me.key() instanceof Symbol && me.val() instanceof Var) {
			...
			if (!exclude.contains(k)) {
				v.alterMeta(Var.assoc, list(SHARED_KEY, T));
			} else {
				v.alterMeta(Var.assoc, list(SHARED_KEY, F));
			}
		}
	}
}
...
synchronized public static DynamicClassLoader getRootClassLoader() {
	if (ROOT_CLASSLOADER == null) {
		...
		((com.tc.object.loaders.NamedClassLoader) ROOT_CLASSLOADER)
				.__tc_setClassLoaderName("Clojure Classloader");
		com.tc.object.bytecode.hook.impl.ClassProcessorHelper
				.registerGlobalLoader((com.tc.object.loaders.NamedClassLoader) ROOT_CLASSLOADER);
	}
	return ROOT_CLASSLOADER;
}
}