...
import clojure.lang.Var.Frame;


public final class VarTC extends ARef implements IFn, IRef, Settable{
...
static Keyword sharedKey = Keyword.intern(null, "tcshared");
...
VarTC(Namespace ns, Symbol sym){
	this.ns = ns;
	this.sym = sym;
	this.count = new AtomicInteger();
	this.root = NOT_BOUND;  //use NOT_BOUND as magic not-bound value
	this.sharedRoot = NOT_BOUND;  //use NOT_BOUND as magic not-bound value
	setMeta(PersistentHashMap.EMPTY);
}

VarTC(Namespace ns, Symbol sym, Object root){
	this(ns, sym);
	this.root = root;
}
...
private void _setRoot(Object root) {
	if (isShared()) {
...
private synchronized void adjustSharing(boolean wasShared) {
	if (wasShared && !isShared()) {
		this.root = this.sharedRoot;
		this.sharedRoot = NOT_BOUND;
	} else if (!wasShared && isShared()) {
		this.sharedRoot = this.root;
		this.root = NOT_BOUND;
	}
}
...
public synchronized IPersistentMap alterMeta(IFn alter, ISeq args)
		throws Exception {
	boolean wasShared = isShared();
	IPersistentMap retval = super.alterMeta(alter, args);
	adjustSharing(wasShared);
	return retval;
}

public synchronized IPersistentMap resetMeta(IPersistentMap m) {
	boolean wasShared = isShared();
	IPersistentMap retval = super.resetMeta(m);
	adjustSharing(wasShared);
	return retval;
}
...
public boolean isShared(){
	return meta() != null && RT.booleanCast(meta().valAt(sharedKey));
}
...
final public boolean hasLocalRoot(){
	return !isShared() && root != NOT_BOUND;
}

final public boolean hasSharedRoot(){
	return isShared() && sharedRoot != NOT_BOUND;
}

static boolean printing = false;

final public boolean hasRoot(){
	return hasLocalRoot() || hasSharedRoot();
}
